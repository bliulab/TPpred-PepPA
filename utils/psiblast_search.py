...
def gen_nr(input_fasta, target_folder, db):
    tmp_folder = target_folder + 'tmp/'
    pssm_folder = target_folder + 'pssm/'
    ...
    for i in trange(len(names)):
        ...
        gen_pssm_by_blast(src, pssm_folder, xml_folder, db)



def run(input_fasta, target_folder, db, nr = None):
    """
    ...
    if nr is not None:
        for i in trange(len(rest_names)):
            fname = rest_names[i]
            src = tmp_folder + fname + '.fasta'
            gen_pssm_by_blast(src, pssm_folder, xml_folder, nr)

    # blosum
    rest_names = check(names, pssm_folder)
    print(print('blosum:', len(rest_names)))
...
def gen_pssm_by_blast(src, pssm_folder, xml_folder, db):
    """
    """
    name = src.split('/')[-1][:-6]
    ...
    if sys.platform.startswith('win'):
        psiblast_cmd = '.\\utils\\psiblast\\psiblast.exe'

    else:
        psiblast_cmd = './utils/psiblast/psiblast'

    evalue_threshold = 0.01    # 最后确实是0.01
    num_iter = 3
    outfmt_type = 5
