...
class FocalDiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        #检查 预测值（input） 和 目标标签（target） 的 批次大小是否相同
        assert input.shape[0] == target.shape[0], "predict & target batch size don't match"
        predict = nn.Sigmoid()(input)
        # predict = input
        # [batch,label_nums] -->[batch,21]
        predict = predict.contiguous().view(predict.shape[0], -1)
        target = target.contiguous().view(target.shape[0], -1)
...
class VarifocalLoss(nn.Module):
    ...
    Args:
        alpha (float): Balance factor for negative samples.
        gamma (float): Focusing parameter for modulating factor.
        iou_weighted (bool): If True, weight positive loss by target (e.g., IoU).
        use_sigmoid (bool): If True, apply sigmoid to predictions.
        reduction (str): Specifies the reduction to apply to the output: 'none' | 'mean' | 'sum'.
    """
    def __init__(self, alpha=0.75, gamma=2.0, iou_weighted=True, use_sigmoid=True, reduction='mean'):
    ...
    def forward(self, pred, target):
        assert pred.shape == target.shape, "Shape of pred and target must match"
        ...
        if self.iou_weighted:
            focal_weight = target * pos_mask + \
                           self.alpha * (pred_prob - target).abs().pow(self.gamma) * neg_mask
        else:
            focal_weight = pos_mask + \
                           self.alpha * (pred_prob - target).abs().pow(self.gamma) * neg_mask
        ...
        if self.use_sigmoid:
            ...
            loss = loss.sum(dim=1)  # if non-sigmoid and multi-class
